$(document).ready(function(){
	...
	var content_projects = $("#projects-viewer-image, #projects-viewer-info, #projects-slideshow-container, #projects-header")
	...
	$window.on('scroll', function() {
		...
		if ( $window.scrollTop() >= 1530 && $window.scrollTop() < 2284) {
			content_projects.fadeIn();
			content_about.fadeOut();
			content_exp.fadeOut();
			content_contact.fadeOut();
		}		
		if ( $window.scrollTop() >= 2284 && $window.scrollTop() < 3000) {
			content_exp.fadeIn();
			content_projects.fadeOut();
			content_contact.fadeOut();
		}	
		...
		if ( $window.scrollTop() >= about_top && $window.scrollTop() < projects_top) {
			$('.menu-item').removeClass('active');
	...
	function showProject(project_active){	
		...
		for(var i = 0; i < projects.length; i++){
			$(projects[i]).find('.project-image').fadeOut();
			$(projects[i]).find('.project-info').fadeOut();
			$(projects[i]).removeClass('active');
		}
		$(project_active).addClass('active');

		$(project_image).clone().appendTo('#projects-viewer-image').fadeIn();
		$(project_info).clone().appendTo('#projects-viewer-info').fadeIn();
		
	    selected = $('#projects-viewer-image').find('.project-image');
		x = $(selected).find('.mySlides');  
		dots = $(selected).find('.carousel-buttons').find('.item');
	...
	var width = "17vw";
	console.log($('#projects-slides').offset().left);
	//Arrow events
	$('#left-arrow').click(function(){
		console.log($('#projects-slides').offset().left);
		if ($('#projects-slides').offset().left <= 60){
			$('#projects-slides').animate({"margin-left": "+="+width},animationSpeed);
		}
	});
	$('#right-arrow').click(function(){
		console.log($('#projects-slides').offset().left);
		if ($('#projects-slides').offset().left >= -230){
			$('#projects-slides').animate({"margin-left": "-="+width},animationSpeed);
		}
	});
	...
	var exp_content = $(".exp-section ul");
